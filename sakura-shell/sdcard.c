...
/* Measured and tuned values. */
#define COMMAND_FINISH_CYCLE_MAX 10
#define CMD0_FINISH_CYCLE_MAX    10
#define ACMD41_FINISH_CYCLE_MAX  10
#define CMD17_FINISH_CYCLE_MAX   32
...
void sdcard_info_print(void)
{
  ...
  response = spi_sdcard_command(17, 0x00000000);
  if (response != 0) {
    cdc_send("\r\nCMD17 failed!");
  }
  ...
  /* Dump block 0 (MBR) in hex. */
  text[0] = '\r';
  text[1] = '\n';
  for (int i = 0; i < 32; i++) {
    for (int j = 0; j < 48; j += 3) {
      response = spi_transfer(0xff);
      text[j + 2] = ((response & 0xF0) >> 4) + 0x30;
      if (text[j + 2] > 0x39) text[j + 2] += 7;
      text[j + 3] =  (response & 0x0F) + 0x30;
      if (text[j + 3] > 0x39) text[j + 3] += 7;
      text[j + 4] = ' ';
    }
    text[49] = '\0';
    cdc_send(text);
